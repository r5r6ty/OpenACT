...
--~ 		for i = self.frame, #self.database[self.id][self.action].frames, 1 do
			...
			elseif currentFrame.category == "Command" then
				table.insert(self.commandQueue, {name = currentFrame.command, rangeA = currentFrame.rangeA, rangeB = currentFrame.rangeB, wait = currentFrame.wait, waitCounter = 0})
				self.frame = self.frame + 1
...
--~ 	end
	...
	-- 命令计时
	local ccount = 1
	local csize = #self.commandQueue
	while ccount <= csize do
		if self.commandQueue[ccount].wait ~= 0 then
			if self.commandQueue[ccount].waitCounter >= self.commandQueue[ccount].wait then
				table.remove(v, ccount)
				ccount = ccount - 1
				csize = csize - 1
			end
			self.commandQueue[ccount].waitCounter = self.commandQueue[ccount].waitCounter + 1
		end
		ccount = ccount + 1
	end
